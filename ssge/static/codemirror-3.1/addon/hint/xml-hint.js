(function() {
    ...
    CodeMirror.xmlHint = function(cm) {
        ...
        if (cursor.ch > 0) {
            ...
            // only direct parrent tag matters
            path = path.slice(getActiveElement(text).lastIndexOf('<'), path.length)
            ...
            if(typeof hints === 'undefined')
                ...
                for (var i = hints.length - 1; i >= 0; i--) {
                    if(hints[i].hint.indexOf(typed) != 0)
                        hints.splice(i, 1);
                }
            }
    ...
    var getOpenTag = function(text) {
        ...
        if (close < open)
        {
            ...
            if(text != '<') {
                // oval identifier hinting workaround
                if (text.match('<oval.* id=$'))
                    return '<oval id';

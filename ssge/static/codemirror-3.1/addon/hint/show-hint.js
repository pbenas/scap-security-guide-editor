CodeMirror.showHint = function(cm, getHints, options) {
  ...
  function showHints(data) {
    ...
    if (!continued && options.completeSingle !== false && completions.length == 1) {
      cm.replaceRange(completions[0].hint, data.from, data.to);
      return true;
    }
    ...
    for (var i = 0; i < completions.length; ++i) {
      ...
      elt.title = completions[i].help;
      elt.appendChild(document.createTextNode(completions[i].hint));
      elt.hintId = i;
      elt.style.padding = "0px 15px 0px 0px"
    }
    ...
    function pick() {
      cm.replaceRange(completions[selectedHint].hint, data.from, data.to);
      close();
    }
